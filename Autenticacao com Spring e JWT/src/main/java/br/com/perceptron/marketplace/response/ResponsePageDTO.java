...
import br.com.perceptron.marketplace.paginacao.PageInfo;
...
public class ResponsePageDTO<T> implements Serializable {
    ...
    private ResponsePageDTO(String message, Page<T> page) {
        this.dateTime = DateUtils.getDefaultBR();
        this.message = message;
    ...
    public ResponseEntity<Object> success(HttpStatus status, String message, Page<T> page, Class<?> mClass) {
    ...
    private void registrarLogDeErro(HttpStatus status, String error, Class<?> mClass, String method, Exception e, HttpHeaders headers, WebRequest request) {
        //FIXME criar table de parametros para salvar se imprime ou nao log de erro
    ...
    private void registrarLogDeSucesso(HttpStatus status, String message, Object object, Class<?> mClass) {
        //FIXME criar table de parametros para salvar se imprime ou nao log de sucesso
        if (false) new Logs(mClass).info(status, message, object.toString(), mClass);
    }
}
