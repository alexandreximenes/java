...
function addOracleInfinity() {
  log("global.js ora infinity load");
  var infiScript = document.createElement('script');
  infiScript.type = "text/javascript";
  if(window.location.hostname =="docs.oracle.com"){
    log("global.js ora infinity load -prod");
    //infiScript.src = "https://c.oracleinfinity.io/acs/account/wh3g12c3gg/js/oracledocs/odc.js?_ora.context=analytics:production"; not yet ready for prod
  } else {
      log("global.js ora infinity load - dev")
...
function tryFramedFooters() {
  ...
  for (var i = 0; i < frames.length; i++) {
    try {
       ...
       * calling routine knows to avoid a hail mary banner
       *************************/
